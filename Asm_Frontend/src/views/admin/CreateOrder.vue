...
const getAddressByUser = async () => {
  try {
    ...
    const resp = await axios.get("/api/v1/address/" + userId);
...
const getOrderDetailById = async (idOrder) => {
  try {
    const resp = await axios.get("/api/v1/orders-detail/GetByOrderId/" + idOrder);
...
const getOrderById = async () => {
  try {
    const resp = await axios.get("/api/v1/orders/" + route.params.idOrder);
    formOrder.value = {
...
const getAllOrderStatus = async () => {
  try {
    const resp = await axios.get("/api/v1/order-status/List");
...
const saveOrder = async () => {
  ...
  try {
    ...
    if (!isEdit.value) {
      ...
      resp = await axios.post("/api/v1/orders/", newOrder);
      if (resp.data.result) {
      ...
      const resp = await axios.put(`/api/v1/orders/${route.params.idOrder}`, updateOrder);
  ...
  try {
    ...
    if (creates.length > 0) {
      for (let data of creates) {
        await axios.post("/api/v1/orders-detail/", data);
      }
    }

    if (deletes.length > 0) {
      for (let data of deletes) {
        await axios.delete(`/api/v1/orders-detail/${data.id}`);
      }
    }

    for (let data of updates) {
      // console.log(data);
      await axios.put(`/api/v1/orders-detail/${data.id}`, data);
    }
  } catch (error) {
    console.log(error.message);
  }
