...
const getProductById = async () => {
  try {
    const resp = await axios.get(`/api/v1/product/${route.params.idProduct}`);
...
const getAllCategory = async () => {
  try {
    const resp = await axios.get("/api/v1/category/List");
...
const getAllSpecGroup = async () => {
  try {
    const resp = await axios.get("/api/v1/specificationType/List");
...
const getAllProductSpec = async () => {
  try {
    const resp = await axios.get("/api/v1/product-specification/List");
...
const getAllProducType = async () => {
  try {
    const resp = await axios.get("/api/v1/productType/List");
...
const saveProduct = async () => {
  try {
    ...
    if (isEdit.value) {
      ...
      resp = await axios.put(`/api/v1/product/${currentIdProduct}`, product.value);
      ...
      // Tạo sản phẩm mới
      resp = await axios.post("/api/v1/product/", newProduct);
...
const saveProductSpec = async () => {
  try {
    ...
    for (let data of dataPost) {
      ...
      const resp = await axios.post("/api/v1/product-specification/", data);
    }
  } catch (error) {
    console.log(error.message);
  }
  ...
  try {
    ...
    if (creates.length > 0) {
      for (let data of creates) {
        if (data.value == "") data.value = "Không có";
        await axios.post("/api/v1/product-specification/", data);
      }
    }

    if (deletes.length > 0) {
      for (let data of deletes) {
        await axios.delete(`/api/v1/product-specification/${data.id}`);
      }
    }

    if (updates.length > 0) {
      for (let data of updates) {
        await axios.put(`/api/v1/product-specification/${data.id}`, data);
      }
    }
  } catch (error) {
...
const saveImage = async (idProduct) => {
  ...
  try {
    await axios.post(`/api/v1/image/${idProduct}`, formData, {
...
const deleteImage = async () => {
  ...
  try {
    await axios.delete(`/api/v1/image/${route.params.idProduct}`, {
      data: requestData,
