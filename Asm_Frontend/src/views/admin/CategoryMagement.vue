...
const pageinatedCategories = async () => {
  try {
    const resp = await axios.get("/api/v1/category/Get", {
      params: {
        size: limit.value,
...
const createCategory = async () => {
  try {
    ...
    const resp = await axios.post("/api/v1/category/", newCategory);
    if (resp.data.result) {
...
const editCategory = async () => {
  try {
    ...
    const resp = await axios.put(`/api/v1/category/${id}`, { categoryName, parentCategory });
