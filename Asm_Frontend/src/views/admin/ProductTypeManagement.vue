...
const pageinatedProductType = async () => {
  try {
    const resp = await axios.get("/api/v1/productType/Get", {
      params: {
        size: limit.value,
...
const createProductType = async () => {
  try {
    ...
    const resp = await axios.post("/api/v1/productType/", newProductType);
    if (resp.data.result) {
      ...
      for (let sp of newSpec) {
        await axios.post("/api/v1/specificationType/", sp);
      }
...
const editProductType = async () => {
  try {
    ...
    const resp = await axios.put(`/api/v1/productType/${id}`, { nameType });
    ...
    if (resp.data.result) {
      ...
      if (specCreate.length > 0) {
        for (let sp of specCreate) await axios.post("/api/v1/specificationType/", sp);
      }
      if (specUpdate.length > 0) {
        for (let sp of specUpdate) await axios.put("/api/v1/specificationType/" + sp.id, sp);
      }

      if (specDelete.length > 0) {
        for (let sp of specDelete) await axios.delete("/api/v1/specificationType/" + sp.id);
      }
      // await pageinatedProductType();
    }
...
const deleteProductType = async (id) => {
  try {
    ...
    await axios.delete(`/api/v1/productType/${id}`);
    await pageinatedProductType();
